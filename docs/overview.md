# repository-manager — Concept Overview
...
```
repository-manager/
├── repository_manager/        # Source code
│   ├── __init__.py
...
```json
{
  "mcpServers": {
    "repository-manager": {
      "command": "uv",
      "args": ["run", "--with", "repository-manager", "repository-mcp"],
      "env": {}
    }
  }
}
...
```bash
repository-mcp --transport streamable-http --port 8001
```
