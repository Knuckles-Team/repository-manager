...
def test_recheck_refusal_preserves_tunnel_manager_import_error_as_cause():
    """Refusal proof: tunnel-manager absent -> named refusal, full cause chain (H-12)."""
...
def test_import_mcp_server_without_tunnel_manager():
    """``import repository_manager.mcp_server`` succeeds with tunnel-manager absent.
    ...
    completed = subprocess.run(
        [sys.executable, "-c", "import tunnel_manager"],
        capture_output=True,
        text=True,
        timeout=60,
        env=env,
    ...
    assert completed.returncode != 0, (
        "expected tunnel_manager to be absent in this environment; the "
        "RMDD-20 refusal proof requires it"
    )
