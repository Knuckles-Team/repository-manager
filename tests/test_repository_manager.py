...
from repository_manager.repository_manager import Git, GitError
...
class TestDirectoryFunctions:
    ...
    def test_create_directory_success(self, mock_exists, mock_makedirs, git_instance):
        mock_exists.return_value = False # Directory does not exist, project exists (handled in logic)
        ...
        def exists_side_effect(path):
            if path.endswith("myproject"): return True
            return False
            
        mock_exists.side_effect = exists_side_effect

        result = git_instance.create_directory(workspace=git_instance.workspace, project="myproject", path="newdir")
    ...
    def test_create_directory_already_exists(self, mock_exists, git_instance):
        ...
        result = git_instance.create_directory(workspace=git_instance.workspace, project="myproject", path="existingdir")
    ...
    def test_create_directory_project_not_found(self, mock_exists, git_instance):
        ...
        result = git_instance.create_directory(workspace=git_instance.workspace, project="missingproject", path="newdir")
    ...
    def test_create_directory_outside_workspace(self, git_instance):
        ...
        result = git_instance.create_directory(workspace=git_instance.workspace, project=None, path="../outside")
    ...
    @patch("os.path.exists")
    def test_delete_directory_success_dir(self, mock_exists, mock_isfile, mock_remove, mock_rmtree, git_instance):
        mock_exists.return_value = True
    ...
    def test_delete_directory_workspace_root(self, git_instance):
        result = git_instance.delete_directory(path=".") # Relative to workspace, this is workspace root
        assert result.status == "error"
    ...
    def test_rename_directory_success(self, mock_exists, mock_renames, git_instance):
        ...
        def exists_side_effect(path):
            if "old" in path: return True
            if "new" in path: return False
            return False
        mock_exists.side_effect = exists_side_effect
...
class TestTextEditor:
    ...
    def test_view_file_range(self, mock_exists, mock_file, git_instance):
        ...
        # View lines 1-2
        result = git_instance.text_editor(command="view", path="test.txt", view_range=[1, 2])
        assert result.status == "success"
    ...
    def test_create_file(self, mock_exists, mock_makedirs, mock_file, git_instance):
        ...
        result = git_instance.text_editor(command="create", path="new.txt", file_text="hello")
        
        assert result.status == "success"
        mock_file.assert_called_with(os.path.join(git_instance.workspace, "new.txt"), "w")
    ...
    def test_str_replace(self, mock_exists, mock_file, git_instance):
        ...
        result = git_instance.text_editor(command="str_replace", path="file.txt", old_str="world", new_str="universe")
    ...
    def test_str_replace_not_found(self, mock_exists, mock_file, git_instance):
        mock_exists.return_value = True
        result = git_instance.text_editor(command="str_replace", path="file.txt", old_str="universe", new_str="world")
        assert result.status == "error"
...
class TestProjectFunctions:
    ...
    @patch("os.makedirs")
    @patch("os.path.exists")
    def test_create_project_success(self, mock_exists, mock_makedirs, mock_popen, git_instance):
