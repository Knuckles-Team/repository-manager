...
- [repository-manager-parallel-lane-orchestration](../../repository-manager-parallel-lane-orchestration/SKILL.md): Run a program as one orchestrator plus many parallel worker agents — the lane briefing contract, landing each branch the moment it arrives, provider-before-consumer ordering across repos, adjudicating contradictory lane reports, and reconciling the workspace back to zero branches and zero worktrees. Use this when dispatching MORE THAN ONE worker agent at a time.
