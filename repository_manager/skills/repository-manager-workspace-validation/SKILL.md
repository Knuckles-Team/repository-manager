...
description: >-
  ...
  validation per project, install/build ecosystems, and drive phased version bumps
  and maintenance. Use when the agent must ensure all projects are valid, fix
  project errors, run the validation suite, or bump versions across a dependency-
  ordered workspace. Do NOT use for isolated session worktrees
  (repository-manager-worktree-orchestration) or bulk clone/pull/push
  (repository-manager-bulk-git-operations).
license: MIT
tags: [repository-manager, validation, pre-commit, bumpversion, mcp]
metadata:
  author: Genius
  version: '0.1.0'
---
